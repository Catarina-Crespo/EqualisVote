...
let intersectionCompletedUsers = 0;
...
async function voteForUser(voter, votee_upk, wasm, groupUsers) {
  ...
  metrics.voteEncryption.set(voter.upk_u_i, end - start); // store ms
    ...
    ws.on("message", async (data) => {
      ...
      switch (msg.type) {
        ...
        case "int_step_3": {
          ...
          // Increment counter
          intersectionCompletedUsers++;

          // Only set intersectionEnd when all users have finished
          if (intersectionCompletedUsers == 1) {
          ...
          console.log("Group votes retrieved:" + valuesArray.length);
...
function printMetrics() {
  ...
  for (let [upk, bw] of metrics.bandwidth.entries()) {
    const label = upk === externalUserRef.upk_u_i ? " (external user)" : "";
    console.log(
